...
if [ -z ${THEBOX_SERVICE_HOST} ]
then
  MDM_SERVICE_HOST=localhost
fi
if [ -z ${THEBOX_SERVICE_PORT_MARIADB}]
then
  MDM_SERVICE_PORT_MARIADB=3306
fi


sed -i "s/CHANGE_USER/${MYSQL_USER}/g" ${CONF}
sed -i "s/CHANGE_PASSWORD/${MYSQL_PASSWORD}/g" ${CONF}
sed -i "s/CHANGE_SERVICE_HOST/${THEBOX_SERVICE_HOST}/g" ${CONF}
